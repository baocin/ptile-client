...
**Date:** 2026-07-09  
**Repos:** steele.red/ptiles2 + ptiles-client (core/wasm)  
**Status:** Short same-city routes often work. Same-state city pairs (~50–300 km) and cross-state routes largely **fail** or stall. Not production-ready.
...
| UI                   | `steele.red/ptiles2/index.html`                                   | Corridor cells (h3-js), Range I/O, zstd, snap, draw                          |
| Live                 | `https://steele.red/ptiles2/`                                     | S3 + CloudFront; use `?v=N` cache-bust after deploy                          |
...
```bash
cd /home/aoi/kino/projects/ptiles-client/wasm
wasm-pack build --target web \
  --out-dir /home/aoi/kino/projects/steele.red/ptiles2/lib/client \
  --out-name ptiles_client
```

Deploy UI (scoped, never root `--delete`):

```bash
AWS_PROFILE=steele-red-deploy \
  aws s3 sync /home/aoi/kino/projects/steele.red/output/ptiles2/ s3://steele.red/ptiles2/
# invalidate CF dist E1X2E2N30TVNGX paths /ptiles2/*
```
...
## UI knobs (`ptiles2/index.html`)

| Setting           | Current                             | Pain                              |
| ----------------- | ----------------------------------- | --------------------------------- |
| `ROUTE_MAX_CELLS` | 400                                 | Cap for phone; thin spine if over |
| `corridorParams`  | short: w=2 full; long: w=1 arterial | Long same-state still null        |
...
6. **Deploy checklist**  
   Always sync `ptiles2/` + wasm, CF invalidate `/ptiles2/*`, verify live HTML contains `route_from_segments` and `statesAlong`, open with `?v=N`.
...
cd ptiles-client/wasm && wasm-pack build --target web \
  --out-dir ../../steele.red/ptiles2/lib/client --out-name ptiles_client
...
# UI local
cd steele.red && python3 serve.py   # :8000 → /ptiles2/
```

Node smoke pattern used in session: load h3-js UMD + wasm `init({module_or_path: bytes})` + local Range-like reads from TN.roads; call `route_from_segments` with corridor-decoded segs.
