...
function roadStyle(roadClass) {
  const idx = ["motorway","trunk","primary","secondary","tertiary","unclassified","residential","service","path"].indexOf(roadClass);
...
class Layer {
  ...
  async ensureOpen(state) {
    ...
    this.loading = true;
    try {
      this.file = new PtilesRemoteFile(wasmMod, stateLayerUrl(state, this.name));
...
const layers = {
  roads: new Layer("roads", (raw) => {
    ...
    return segs
      ...
      .map((s) => L.polyline(s.coords.map((c) => [c[1], c[0]]), roadStyle(s.road_class)));
  }),
  water: new Layer("water", (raw) => {
    ...
    return feats
      ...
      .map((f) =>
        f.geom_type === 0
          ? L.polygon(f.coords.map((c) => [c[1], c[0]]), { color: "#1f6feb", weight: 1, fillColor: "#1f6feb", fillOpacity: 0.25 })
          : L.polyline(f.coords.map((c) => [c[1], c[0]]), { color: "#1f6feb", weight: 1.5, opacity: 0.7 })
      );
  }),
  parks: new Layer("parks", (raw) => {
    ...
    return feats
      ...
      .map((f) => L.polygon(f.coords.map((c) => [c[1], c[0]]), { color: "#238636", weight: 1, fillColor: "#238636", fillOpacity: 0.2 }));
  }),
  rail: new Layer("rail", (raw) => {
    const feats = wasmMod.decode_rail(raw);
    return feats.map((f) =>
      f.geom_type === 0 && f.coords.length >= 2
        ? L.polyline(f.coords.map((c) => [c[1], c[0]]), { color: "#484f58", weight: 2, opacity: 0.9 })
        : L.circleMarker([f.coords[0][1], f.coords[0][0]], { radius: 4, color: "#484f58", fillColor: "#484f58", fillOpacity: 0.8, weight: 1 })
    );
  }),
  buildings: new Layer("buildings_v8", (raw, cellHex) => {
    const [lat, lon] = wasmMod.cell_center(cellHex);
    const bldgs = wasmMod.decode_buildings(raw, lat, lon);
    const colors = ["#6e40c9", "#388bfd", "#3fb950", "#f0883e", "#da3633", "#d29922"];
    return bldgs
      .filter((b) => b.coords.length >= 3)
      .map((b) => {
        const c = colors[Number(BigInt(b.osm_id) < 0n ? -BigInt(b.osm_id) : BigInt(b.osm_id)) % colors.length];
        return L.polygon(b.coords.map((c2) => [c2[1], c2[0]]), { color: c, weight: 1, fillColor: c, fillOpacity: 0.1 });
      });
  }),
};
...
async function onMapClick(e) {
  ...
  clickMarker = L.circleMarker([lat, lng], { radius: 5, color: "#f0883e", fillColor: "#f0883e", fillOpacity: 0.9 }).addTo(map);
  try {
    const road = await findNearestRoad(lat, lng);
    if (clickHighlight) { map.removeLayer(clickHighlight); clickHighlight = null; }
    const panel = document.getElementById("infoPanel");
    if (!road) {
      panel.classList.add("show");
      document.getElementById("infoBody").textContent = "No road found within threshold.";
      setStatus("no road found");
      return;
    }
    clickHighlight = L.polyline(road.geometry, { color: "#3fb950", weight: 4, opacity: 0.9 }).addTo(map);
    panel.classList.add("show");
    ...
    `;
    setStatus(`nearest road: ${road.name || road.road_class} (${road.distance_m.toFixed(1)} m)`);
  } catch (err) {
...
async function searchBusinesses(state, query, limit) {
  ...
  try {
    const idx = new PtilesRemoteFile(wasmMod, stateLayerUrl(state, "business_name_index"));
    await idx.open();
    ...
    const hits = raw ? wasmMod.match_business_name_block(raw, query, limit) : [];
    renderSearchResults(hits);
    setStatus(`${hits.length} result(s) (indexed)`);
    return;
  } catch (e) {
    setLayerStatus(`name index unavailable (${e.message}), falling back to brute force`);
  }
...
function renderSearchResults(hits) {
  ...
  for (const hit of hits) {
    ...
    div.addEventListener("click", () => {
      ...
      if (clickMarker) map.removeLayer(clickMarker);
      clickMarker = L.circleMarker([hit.lat, hit.lon], { radius: 6, color: "#d29922", fillColor: "#d29922", fillOpacity: 0.9 }).addTo(map);
    });
...
async function main() {
  ...
  map = L.map("map").setView(STATE_CENTERS[currentState].center, STATE_CENTERS[currentState].zoom);
  ...
  Object.keys(STATE_CENTERS).sort().forEach((s) => {
