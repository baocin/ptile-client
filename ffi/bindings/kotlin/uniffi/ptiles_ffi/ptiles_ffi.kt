...
fun uniffi_ptiles_ffi_checksum_method_namescanner_len(
): Short
fun uniffi_ptiles_ffi_checksum_method_namescanner_search(
): Short
...
fun uniffi_ptiles_ffi_checksum_method_ptileslayer_name_scan(
): Short
...
fun uniffi_ptiles_ffi_fn_clone_namescanner(`ptr`: Pointer,uniffi_out_err: UniffiRustCallStatus, 
): Pointer
fun uniffi_ptiles_ffi_fn_free_namescanner(`ptr`: Pointer,uniffi_out_err: UniffiRustCallStatus, 
): Unit
fun uniffi_ptiles_ffi_fn_method_namescanner_len(`ptr`: Pointer,uniffi_out_err: UniffiRustCallStatus, 
): Int
fun uniffi_ptiles_ffi_fn_method_namescanner_search(`ptr`: Pointer,`query`: RustBuffer.ByValue,`limit`: Int,uniffi_out_err: UniffiRustCallStatus, 
): RustBuffer.ByValue
...
fun uniffi_ptiles_ffi_fn_method_ptileslayer_name_scan(`ptr`: Pointer,uniffi_out_err: UniffiRustCallStatus, 
): RustBuffer.ByValue
...
private fun uniffiCheckApiChecksums(lib: IntegrityCheckingUniffiLib) {
    ...
    if (lib.uniffi_ptiles_ffi_checksum_method_namescanner_len() != 1367.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
    if (lib.uniffi_ptiles_ffi_checksum_method_namescanner_search() != 57967.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
    ...
    if (lib.uniffi_ptiles_ffi_checksum_method_ptileslayer_name_scan() != 40760.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
    ...
    if (lib.uniffi_ptiles_ffi_checksum_method_ptileslayer_search_business_everywhere() != 10942.toShort()) {
        throw RuntimeException("UniFFI API checksum mismatch: try cleaning and rebuilding your project")
    }
 ...
 */
public interface NameScannerInterface {
     ...
     */
    fun `search`(`query`: kotlin.String, `limit`: kotlin.UInt): List<BusinessSearchHit>
 ...
 * it, so the caller holds this and pays the 132 ms once.
 */
open class NameScanner: Disposable, AutoCloseable, NameScannerInterface
{
    ...
    private class UniffiCleanAction(private val pointer: Pointer?) : Runnable {
        override fun run() {
            pointer?.let { ptr ->
                uniffiRustCall { status ->
                    UniffiLib.INSTANCE.uniffi_ptiles_ffi_fn_free_namescanner(ptr, status)
                }
            }
        }
    }
    ...
    fun uniffiClonePointer(): Pointer {
        return uniffiRustCall() { status ->
            UniffiLib.INSTANCE.uniffi_ptiles_ffi_fn_clone_namescanner(pointer!!, status)
        }
    }
    ...
    callWithPointer {
    uniffiRustCall() { _status ->
    UniffiLib.INSTANCE.uniffi_ptiles_ffi_fn_method_namescanner_len(
        it, _status)
}
    }
    )
     ...
     * with the same rule the builder folded the names with, so a caller
     * passes what the user typed.
     */override fun `search`(`query`: kotlin.String, `limit`: kotlin.UInt): List<BusinessSearchHit> {
            return FfiConverterSequenceTypeBusinessSearchHit.lift(
    callWithPointer {
    uniffiRustCall() { _status ->
    UniffiLib.INSTANCE.uniffi_ptiles_ffi_fn_method_namescanner_search(
 ...
 * @suppress
 */
public object FfiConverterTypeNameScanner: FfiConverter<NameScanner, Pointer> {

    override fun lower(value: NameScanner): Pointer {
        return value.uniffiClonePointer()
    }

    override fun lift(value: Pointer): NameScanner {
        return NameScanner(value)
    }

    override fun read(buf: ByteBuffer): NameScanner {
        // The Rust code always writes pointers as 8 bytes, and will
        // fail to compile if they don't fit.
        return lift(Pointer(buf.getLong()))
    }

    override fun allocationSize(value: NameScanner) = 8UL

    override fun write(value: NameScanner, buf: ByteBuffer) {
        // The Rust code always expects pointers written as 8 bytes,
        // and will fail to compile if they don't fit.
        buf.putLong(Pointer.nativeValue(lower(value)))
    }
...
open class PtilesLayer: Disposable, AutoCloseable, PtilesLayerInterface
{
     ...
     * complete and slow -- both of which this replaces where it exists.
     */
    @Throws(PtilesException::class)override fun `nameScan`(): NameScanner? {
            return FfiConverterOptionalTypeNameScanner.lift(
    callWithPointer {
    uniffiRustCallWithError(PtilesException) { _status ->
    UniffiLib.INSTANCE.uniffi_ptiles_ffi_fn_method_ptileslayer_name_scan(
        it, _status)
}
    }
    )
 ...
 * @suppress
 */
public object FfiConverterOptionalTypeNameScanner: FfiConverterRustBuffer<NameScanner?> {
    override fun read(buf: ByteBuffer): NameScanner? {
        if (buf.get().toInt() == 0) {
            return null
        }
        return FfiConverterTypeNameScanner.read(buf)
    }

    override fun allocationSize(value: NameScanner?): ULong {
        if (value == null) {
            return 1UL
        } else {
            return 1UL + FfiConverterTypeNameScanner.allocationSize(value)
        }
    }
    ...
    override fun write(value: NameScanner?, buf: ByteBuffer) {
        if (value == null) {
            ...
            buf.put(1)
            FfiConverterTypeNameScanner.write(value, buf)
        }
    }
}
